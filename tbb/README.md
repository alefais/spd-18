...
## 02 - Mandelbrot Set
Implementation of the [Mandelbrot set](https://en.wikipedia.org/wiki/Mandelbrot_set), i.e. the set of all points *c* in the complex plane that do not go to infinity when the quadratic function *z = z<sup>2</sup> + c* is iterated. <ul><li><b>Version 1:</b> sequential implementation that uses the [SDL2](https://wiki.libsdl.org/) library to draw in a separated window all the images produced by iterating the Mandelbrot set computation.
#### Requirements
